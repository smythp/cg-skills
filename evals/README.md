...
Adapted from the test harness in Adrian Mouat's dfc-skillz
(github.com/amouat/dfc-skillz): the runner shape with its ERROR/FAIL split
and per-fixture smoke scripts, the org substitution, the knobs, and all six
fixtures' inputs are his. The expected outputs here were regenerated by this
skill, and the gate checks, time bounds, per-run naming, and
ephemeral-loopback ports were added for it.
